...
import GrayEditBtn from "../../images/editButton.png";
...
import { ArticleCardVideo, YoutubePlayer } from "../postDetail/postDetailEle";
...
const UserProfile = () => {
  ...
  /* const [waifuInputChange, setWaifuInputChange] = useState([]);
  const [trackWaifuSearch, setTrackWaifuSearch] = useState(false); */
  ...
  const [avaToolTip, setAvaToolTip] = useState(false);
  ...
  // Track when a user change their info
  const [trackUserChanges, setTrackUserChanges] = useState(false);

  // Timeout
  const initTimer = 3600000; //in miliseconds, 3600000 = 1 hour

  // Control styling for the input boxes
  const [isNewNameFocused, setIsNewNameFocused] = useState(false);
  const [isNewSignFocused, setIsNewSignFocused] = useState(false);
  ...
  const handleCancelPreviewImage = () => {
    ...
    if (ratio > 1) {
      xTransformValue = dimensions.width / 2 - 160;
      yTransformValue = dimensions.height / 2 - 160 / ratio;
    } else {
      xTransformValue = dimensions.width / 2 - 100 * ratio;
      yTransformValue = dimensions.height / 2 - 100;
    }
    setImgTransformX(xTransformValue);
    setImgTransformY(yTransformValue);
    setScaleValue(0.3); */
  };
  ...
  const showAvaModal = (e) => {
    e.preventDefault();
    e.stopPropagation();
    const avaModalContainer = document.querySelector(".ava-modal-container");
    avaModalContainer.classList.remove("dialog-gone");
  ...
  const handleOkAva = async () => {
    ...
    myFunction();
    const avaModalContainer = document.querySelector(".ava-modal-container");
    const avaModal = document.querySelector(".ava-modal");
    avaModal.classList.add("modal-leave");
    avaModalContainer.classList.add("dialog-leave");
    setTimeout(() => {
      avaModalContainer.classList.add("dialog-gone");
      avaModalContainer.classList.remove("dialog-leave");
      avaModal.classList.remove("modal-leave");
    }, 200);
    setTrackUserChanges(!trackUserChanges);
  };
  ...
  const showBgModal = (e) => {
    e.stopPropagation();
    e.preventDefault();
    const bgModalContainer = document.querySelector(".bg-modal-container");
    bgModalContainer.classList.remove("dialog-gone");
  ...
  const handleOpenConfirmBox = (index, postId) => {
    const messageBox = document.querySelector(".msg-box-in");
    messageBox.classList.remove("msg-box-gone");
    setConfirmBoxMsg("Xác nhận xóa bài viết?");
  ...
  const handleOkBg = async () => {
    ...
    bgModal.classList.add("modal-leave");
    bgModalContainer.classList.add("dialog-leave");
    setTimeout(() => {
      bgModalContainer.classList.add("dialog-gone");
      bgModalContainer.classList.remove("dialog-leave");
      bgModal.classList.remove("modal-leave");
    }, 200);
    setTrackUserChanges(!trackUserChanges);
  };
  ...
  if (!usersLoading) {
    ...
    if (userIndex === -1) {
      if (user.userid !== userParamIdInt) navigate("/404");
    } else {
  ...
  if (postsLoading) {
    ...
    const likesCounter = myPost.reduce((value, item) => {
      const likes = item.postLikes.filter((p) => p !== user.userid).length;
      return value + likes;
    }, 0);

    const userCreatedTime = new Date(user.createdAt).getTime();
    let seconds = Math.round(Math.abs(currentTime - userCreatedTime) / 1000);
                      ...
                      <ArticleCardAction>
                        ...
                        <PostSelectMenu isActive={postSelectState[i].state}>
                          ...
                          <SelectMenuList>
                            ...
                            <SelectMenuItem
                              onClick={() => handleOpenConfirmBox(i, p.postid)}
                            >
                              <TrashFillIcon />
                              <SelectMenuItemSpan>
                                Xóa bài viết
                                ...
                                >
                                  <ArticleCardMark>
                                    {p.postImage.length > 3 && (
                                      <ArticleImageNum>
                                        <ArticleImageNumIcon />
                                        <span>+{p.postImage.length - 3}</span>
                                      </ArticleImageNum>
                                    )}
                                  </ArticleCardMark>
                                </ArticleCardImageItem>
    ...
    if (!usersLoading) {
      const userWhoFollowed = users.reduce((value, item) => {
        const isFollowed = item.follow.includes(user.userid);
        if (isFollowed) {
          return value + 1;
        } else return value;
      }, 0);
        ...
        <UserActivities>
          ...
          <UARecord>
            ...
            <UAItem>
              ...
              <UAItemValue>{user.follow.length}</UAItemValue>
            </UAItem>
            <UAItem>
  ...
  if (waifusLoading) {
    ...
    /* user.waifulist = user.waifulist.map((waifuId) => {
      if (waifuId !== undefined) {
    ...
    /* user.wishlist = user.wishlist.map((waifuId) => {
      if (waifuId !== undefined) {
        const searchIndex = waifus.findIndex(
          (item) => item._id === waifuId
        );
  ...
  return (
    <>
            ...
            <MainPage>
                  ...
                  <PageHeaderWrap isLined={visible}>
                    <PageHeaderContentLine isLined={visible}>
                      <PageHeaderMain>
                        ...
                        <PageHeaderRight>
                          <AccountCenterAction onClick={(e) => showBgModal(e)}>
                            {tempUserDisp && (
                              <AccountCenterActionImg
                                src={GrayEditBtn}
                                alt="edit-user"
              ...
              <div className="account-center-info">
                ...
                <AccountCenter
                  ...
                  onClick={(e) =>
                    handlePreviewImage(e, bgImage, `user-background`)
                  } */
                  bgImage={bgImage}
                >
                  <AccountCenterHeaderWrap>
                    <AccountCenterHeaderTop>
                      <AccountCenterHeaderAvatar
                        onClick={(e) => showAvaModal(e)}
                        onMouseOver={() => setAvaToolTip(true)}
                        onMouseOut={() => setAvaToolTip(false)}
                      >
                        <AccountAva>
                          <AccountAvaImage src={userAva} alt="user-ava" />
                          <AccountAvaFrame src={AvaFrame} alt="ava-frame" />
                        </AccountAva>
                        <AccountHeaderAvaTooltip hovered={avaToolTip}>
                          Nhấn để thay đổi avatar
                        </AccountHeaderAvaTooltip>
                      </AccountCenterHeaderAvatar>
                      <AccountHeaderButton>
                        <EditButton onClick={(e) => handlerModalOpen(e)}>
                          Thiết lập
                        </EditButton>
                      </AccountHeaderButton>
                    </AccountCenterHeaderTop>
                    <AccountCenterUserInfo>
                        ...
                        <AccountTitle>
                          ...
                          {user.title[user.title.length - 1] && (
                            <span
                              onClick={handleOpenTitle}
                              className={
                                user.title[user.title.length - 1].className
                              }
                            >
                              {user.title[user.title.length - 1].titleName}
                            </span>
                          )}
                        </AccountTitle>
                      </div>
                      <AccountUID>
                        <UserIDIcon />
                        <IDTitle>ID người dùng: {user.userid}</IDTitle>
                      </AccountUID>
                      <AccountUID>
                        <UserSignIcon />
      ...
      <div className="ava-modal-container dialog-container dialog-in dialog-gone">
        <div className="ava-modal modal-in">
          <div className="my-info">Chọn ava bạn muốn thay</div>
          <ImgSelect>
            <SelectedContainer active={AvaRaiden} width={100} borderR={true}>
              <AvaToChoose
            ...
            <SelectedContainer active={AvaHutao} width={100} borderR={true}>
              <AvaToChoose
            ...
            <SelectedContainer active={AvaDoggo} width={100} borderR={true}>
              <AvaToChoose
      ...
      <div className="bg-modal-container dialog-container dialog-in dialog-gone">
        <div className="bg-modal modal-in">
          <div className="my-info">Chọn background bạn muốn thay</div>
          <ImgBgSelect>
            <SelectedContainer active={BgIna} width={180} borderR={false}>
              <BgToChoose
                active={BgIna}
            ...
            <SelectedContainer active={BgLiyue} width={180} borderR={false}>
              <BgToChoose
                active={BgLiyue}
                onClick={handlerBgLi}
                src={LiyueBg}
            ...
            <SelectedContainer active={BgMon} width={180} borderR={false}>
              <BgToChoose
                active={BgMon}
      ...
      {/* <ModalBg visible={bgModalVisible}>
        <div className="my-info">Chọn background bạn muốn thay</div>
        <ImgBgSelect>
          <SelectedContainer active={BgIna} width={180} borderR={false}>
            <BgToChoose
        ...
        <div className="info-modal modal-in">
          ...
          <NewInfoInputDiv>
            <NewInfoInputDivWrap borderChange={isNewNameFocused}>
              ...
              />
              <NewInfoInputMaxCount>{username.length}/100</NewInfoInputMaxCount>
            </NewInfoInputDivWrap>
          </NewInfoInputDiv>
          <AddTitle>Chữ ký</AddTitle>
          <NewInfoInputDiv>
            <NewInfoInputDivWrap borderChange={isNewSignFocused}>
              <NewInfoInputText
                type="text"
                maxLength="100"
                ...
                onBlur={() => setIsNewSignFocused(false)}
                onFocus={() => setIsNewSignFocused(true)}
              />
              <NewInfoInputMaxCount>{usersign.length}/100</NewInfoInputMaxCount>
            </NewInfoInputDivWrap>
          </NewInfoInputDiv>
        </div>
      </div>
      ...
      <TitleModal isOpen={openTitle}>
        ...
        <CurrentProgressTitle>
          ...
          <MyCurrentProgress>{user.roll_count} lần</MyCurrentProgress>
        </CurrentProgressTitle>
        <CurrentProgress>
          <ProgressImgContainer>
            ...
            <AvaArrow
              ...
              percent={
                user.roll_count > 1000
                  ? `80%`
                  : `${(user.roll_count / 10) * 0.8}%`
              }
            />
            <AvaHolder
              isActivated={delayState}
